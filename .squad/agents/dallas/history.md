...
- 2026-05-22T14:24:47Z: Added Render deployment assets with a root `render.yaml`, Render-compatible Dockerfiles for `backend/` and `ai/`, a backend startup entrypoint in `backend/scripts/start.sh`, and a deployment guide in `docs/DEPLOYMENT.md`.
- 2026-05-22T14:24:47Z: Updated backend configuration to accept Render env names (`JWT_SECRET`, `RENDER`), normalize internal AI service host:port values, and handle Render/Postgres URL formats during runtime and Alembic migration startup.
- 2026-05-22T14:24:47Z: Added GitHub Actions CI in `.github/workflows/ci.yml` for backend pytest, AI pytest, and mobile TypeScript checks, and linked deployment/CI details from the root `README.md`.
