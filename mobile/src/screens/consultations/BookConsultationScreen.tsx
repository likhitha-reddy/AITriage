...
export const BookConsultationScreen = ({navigation, route}: Props) => {
  ...
  const sortedDoctors = useMemo(() => [...doctors].sort((left, right) => sortBy === 'rating' ? right.rating - left.rating : left.fee - right.fee), [doctors, sortBy])
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      await consultationService.bookConsultation({doctorId: selectedDoctorId, scheduledAt, notes, triageResultId: route.params?.triageResultId})
    ...
    <KeyboardScreen contentContainerStyle={styles.container}>
      ...
      <View style={styles.section}>
        <View style={styles.sortRow}>
          <Pressable onPress={() => setSortBy('rating')} style={[styles.sortChip, sortBy === 'rating' && styles.sortSelected]}><Text style={[styles.sortText, sortBy === 'rating' && styles.sortTextSelected]}>Top rated</Text></Pressable>
          <Pressable onPress={() => setSortBy('fee')} style={[styles.sortChip, sortBy === 'fee' && styles.sortSelected]}><Text style={[styles.sortText, sortBy === 'fee' && styles.sortTextSelected]}>Lowest fee</Text></Pressable>
        </View>
        {sortedDoctors.map(doctor => <DoctorCard key={doctor.id} doctor={doctor} selected={doctor.id === selectedDoctorId} onPress={() => setSelectedDoctorId(doctor.id)} />)}
      </View>
